...
@Component
public class PreventMultiplePaymentCreation implements PaymentCreatedObserver {

    private static final int MAX_PAYMENT_COUNT_PER_DAY = 5;
    ...
    public PreventMultiplePaymentCreation(PaymentCreatedObserversManager paymentCreatedObserversManager,
    ...
    public void onPaymentCreated(Long paymentId, Long userId) {
        ...
        if (payments.length >= MAX_PAYMENT_COUNT_PER_DAY) {
            paymentMethodService.blockPaymentMethod(current_payment.getMethod(),
                                                    userId,
                                                    LocalDateTime.now().withNano(0).plusDays(RISK_BLOCKING_DAYS));
        }
    }
}